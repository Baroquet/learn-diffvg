...
class SketchRNN(th.nn.Module):
    ...
    class Decoder(th.nn.Module):
        ...
        def forward(self, inputs, z, hidden_and_cell=None):
            ...
            params = self.parameters_predictor(outputs).view(bs, steps, -1)  # 每一个bs，step对应一个6M+3维度参数

            pen_logits = params[..., -3:]  # 每个step的 q1,q2,q3
            gaussian_params = params[..., :-3]  # 每个step所有高斯分布及其权重参数，6M个
            mixture_logits = gaussian_params[..., :self.num_gaussians]  # 前M个是分布权重
            gaussian_params = gaussian_params[..., self.num_gaussians:].view(
                bs, steps, self.num_gaussians, -1)  # 其余所有参数为每个step对应 M*5 个高斯分布参数

            return pen_logits, mixture_logits, gaussian_params, hidden_and_cell  # 每个step，类型、高斯权重分布、高斯参数、lstm decoder输出
    ...
    def forward(self, sequences):
        ...
        # (0, 0, 1, 0, 0)
        z, mu, log_sigma = self.encoder(sequences[:, 1:])  # 编码时跳过每批数据的开始标记命令（0，0，1，0，0）
        ...
        pen_logits, mixture_logits, gaussian_params, hidden_and_cell = \
            self.decoder(sequences[:, :-1], z)  # 解码时跳过每批数据的结束标记命令（dx,dy,0,0,1）
            ...
            point_distrib = \
                th.distributions.multivariate_normal.MultivariateNormal(
                    mu, scale_tril=cov)  # scale_tril(Tensor) -协方差的下三角因子，positive-valued 对角线
                                         # 多元正态分布可以根据正定协方差矩阵或正定精度矩阵或具有positive-valued对角线的下三角矩阵进行参数化
